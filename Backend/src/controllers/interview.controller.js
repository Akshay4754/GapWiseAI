...
async function extractPdfTextFromBuffer(buffer) {
    // Support both pdf-parse APIs used across versions.
    if (pdfParse?.PDFParse) {
        const result = await (new pdfParse.PDFParse(Uint8Array.from(buffer))).getText()
...
async function generateInterViewReportController(req, res) {
    try {
        ...
        if (req.file) {
            ...
            const isPdfByName = typeof req.file.originalname === "string" && req.file.originalname.toLowerCase().endsWith(".pdf")
            ...
            try {
                ...
                // If extraction fails (scanned/secured PDF), continue with a fallback marker.
